...
void CMy1608Dlg::OnButton2() 
{
	...
	password[1]=0x45;
	password[2]=0x45;
	st=IC_CheckWPassword(icdev,1,password);	
	if (st<0)
	{
		m_list1.AddString ("IC_CheckWPassword 1 Error");
		return;
	}
	m_list1.AddString("IC_CheckWPassword 1 ok");

	st=IC_ReadConfigZone_Hex(icdev,20,10,redata);
	if (st<0)
	{
	...
	st=IC_WriteFuse(icdev,3);
	if (st) {
		m_list1.AddString("IC_WriteFuse error");
		return;
	}
	st=IC_ReadFuse(icdev);
	if (st<0) {
		m_list1.AddString("IC_ReadFuse error");
		return;
	}
	...
	password[1]=0x11;
	password[2]=0x11;
	st=IC_CheckWPassword(icdev,0,password);	
	if (st<0)
	{
		m_list1.AddString ("IC_CheckWPassword 0 Error");
		return;
	}
	m_list1.AddString("IC_CheckWPassword 0 ok");
	...
	st=IC_WriteUserZone(icdev,20,10,wrdata);
	if (st<0)
	{
	...
	st=IC_ReadUserZone(icdev,20,10,redata);
	if (st<0)
	{
